...
snapshot_root = './data/snapshot/wifichannel/predcnn2/'
...
class PredCNN(th.nn.Module):
    def __init__(self, sch):
        ...
        self.outconv1 = th.nn.Conv3d(16, 2, kernel_size=3, padding='same', bias=False)
        self.outconv2 = th.nn.Conv3d(16, 2, kernel_size=3, padding='same', bias=False)
        if self.outbn:
    ...
    def forward(self, x, noising=False):
        ...
        if self.outbn:
            x1 = self.obn(x1)
            x2 = self.obn(x2)
        x = self.outact(x1) - self.outact(x2)
